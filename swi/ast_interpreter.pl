...
test(assignment) :-
    % x := 2
    list_to_assoc([a-int(2), x-int(3)], Env),
...
test(assignment2) :-
    % x := y
    list_to_assoc([x-int(2), y-int(3)], Env),
...
test(ite_else) :-
    % if x == 1 then x := 2; y := 2 else x := 3; y := 3
    list_to_assoc([x-int(0), y-int(0)], Env),
    ast_int(if(eq(id(x), int(1)),
               [assign(id(x), int(2)), assign(id(y), int(2))],
               [assign(id(x), int(3)), assign(id(y), int(3))]),
            Env,
